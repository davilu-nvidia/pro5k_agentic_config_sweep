...
2. **P 种子**：纯 PP 优先（平台定律）。PP 深度候选 = 可行的 {浅,中,深} 三档
   ...
   *模型。* 一条 ISL token 的 prompt 按 chunk 切块得到每请求 M = ISL/chunk 个微批；
   操作点并发为 c 时，在场微批总数 N ≈ c×M。S 段流水线处理 N 个微批、每段每批耗时 t，
   完成时间为 (N + S − 1)·t，理想满载为 N·t，故流水效率 = N/(N+S−1)，
   空泡占比 = (S−1)/(N+S−1)。与此独立地，每个块边界要支付一次固定的调度/启动开销
   t_o（本级别机器实测约 10 ms/块），总开销随 M 线性增长。

   *权衡。* 块切小 → N 增大、空泡占比下降，但每多一块多付一次 t_o；块切大则相反。
   取 N ≈ 2S 时效率达 2S/(3S−1) ≈ 2/3 以上——这是效率曲线的拐点：再增加块数，
   每块换回的空泡缩减不足 5%，而 t_o 照付。解 c×(ISL/chunk) = 2S 即得种子：
   ...
   *极限行为（自检）。* S=1（无流水线）：空泡项消失，公式退化为 chunk ≈ c·ISL/2——
   即单卡 prefill 在 c=1 时最优趋向"完全不切块"，与实测一致（没有流水线可填、
   没有并发请求可轮转时，切块是纯开销）。深 S 低 c：公式给出小块，以每块开销
